...
*   **The Dataset:** A synthetic, labeled corpus of 500 constraints (Trusted, Untrusted, and Malicious).
    *   Constraints are seeded from public postmortems, public policy repos, and cloud-provider security/operational guidance — the Kubernetes failure-stories list (k8s.af / hjacobs/kubernetes-failure-stories), the OPA Gatekeeper policy library (open-policy-agent/gatekeeper-library), AWS Security Hub's Foundational Security Best Practices (FSBP) controls reference, Azure Policy built-in definitions, GCP Organization Policy constraints, and the Kyverno policy library (kyverno/policies) — not invented from scratch. This third source family (REVIEW-4 T2.2) also widened provider coverage from kubernetes/terraform-only to all 14 parser-supported providers (aws, azure, gcp, helm, argocd, flux, git, github, sql, mongodb, pulumi, migration, kubernetes, terraform) — see `data/corpus/seeds.yaml`'s header for exact fetch provenance per source.
    ...
    *   **Diversity, honestly:** the 500 constraints contain `n_distinct_structural = 323` distinct `(provider, resource_pattern, actions, scope)` tuples (up from 196 before REVIEW-4 T2.2; 145 distinct patterns, 118 distinct rule texts) — `data/corpus/stats.json`; every place a result reports `n` it reports `n_distinct` beside it. Reached via the third seed source family above plus more variation axes in `scripts/build_corpus.py` (action subsets, occasional effect flips, and provider-appropriate scope keys — `region`/`resource_group`/`subscription`/`namespace`/`env`/`repo`).
...
*   **The Metrics (The Confusion Matrix):**
    ...
    *   **Latency:** p50 and p99 latency of the interception loop. Measured across store sizes (REVIEW-4 T2.3, `scripts/latency_sweep.py`, `results/latency.md`, `docs/latency.svg`): at n=2,000 warm decisions/size (50 warm-up dropped), p50/p99 = 0.017/0.059 ms @100 constraints, 0.051/0.308 ms @500, 0.244/1.393 ms @2,500, 0.926/5.928 ms @10,000 (p99 95% bootstrap CI ±0.03–0.09 ms), with `ConstraintStore.load` at 5.6/24/124/650 ms (no `--sources`) and 7.3/37/197/895 ms (with `--sources`) for the same sizes — the `(provider, action)` index keeps matching sub-linear in store size; load time is the dominant cost at scale.
